	...
	sealed class CopyPasteEditorAction : IEditorAction
	{
		...
		EditorClipboard CopySelectionContents()
		{
			...
			foreach (var cell in source)
			{
				...
				var offset = new CVec(clipboard.CellRegion.TopLeft.X, clipboard.CellRegion.TopLeft.Y);
		...
		public void Do()
		{
			...
			var regionActors = pasteRegion.SelectMany(editorActorLayer.PreviewsAt).Distinct().ToList();
			...
			foreach (var actorKeyValuePair in clipboard.Actors)
			{
				...
				var sourceOffsetVec = new CVec(pastePosition.X - selection.TopLeft.X, pastePosition.Y - selection.TopLeft.Y);
				var actorPosition = locationInit.Value + sourceOffsetVec;
		...
		public void Undo()
		{
			...
			var regionActors = undoClipboard.CellRegion.SelectMany(editorActorLayer.PreviewsAt).Distinct().ToList();
			foreach (var regionActor in regionActors)
			{
				editorActorLayer.Remove(regionActor);
			}
