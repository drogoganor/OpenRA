...
{
	[ChromeLogicArgsHotkeys("ToggleGridOverlayKey", "ToggleBuildableOverlayKey", "ToggleMirrorOverlayKey")]
	public class MapOverlaysLogic : ChromeLogic
	{
		...
		public MapOverlaysLogic(Widget widget, World world, ModData modData, WorldRenderer worldRenderer, Dictionary<string, MiniYaml> logicArgs)
		{
			...
			var toggleMirrorKey = new HotkeyReference();
			if (logicArgs.TryGetValue("ToggleMirrorOverlayKey", out yaml))
				toggleMirrorKey = modData.Hotkeys[yaml.Value];
			...
			keyhandler.AddHandler(e =>
			{
				...
				if (toggleMirrorKey.IsActivatedBy(e))
				{
					mirrorLayerTrait.Enabled ^= true;
					return true;
				}
		...
		Widget CreateOverlaysPanel()
		{
			...
			foreach (var cat in allCategories)
			{
				...
				{
					category.IsChecked = () => mirrorLayerTrait.Enabled;
					category.OnClick = () => mirrorLayerTrait.Enabled ^= true;
				}
